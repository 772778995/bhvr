...
写作要求：
- 使用 Markdown
- 总字数控制在 300 字以内，保持紧凑、可快速扫读
- 语言理性、准确、精炼，不写空洞赞美
- 不要补充作者背景、出版信息、外部评价或延伸资料
- 所有内容必须基于笔记本中的文档来源，不得凭空捏造`;

const BUILTIN_DEEP_READING_PROMPT = `请忽略此前任何对话、提问、回答和中间研究结论，只根据本笔记本当前来源中的文档内容，输出一份中文《详细解读》。
...
写作要求：
- 使用 Markdown
- 总字数控制在 5000 字以内，保证结构清楚、10 分钟左右可读完
- 分段和分点清晰，拒绝机械注水和重复改写
- 语言理性、准确、精炼，不发散
- 不要推荐额外书单或外部资料
- 所有内容必须基于笔记本中的文档来源，不得凭空捏造`;
...
const LEGACY_BUILTIN_QUICK_READ_PROMPT = `请忽略此前任何对话、提问、回答和中间研究结论，只根据本笔记本当前来源中的文档内容，输出一份中文《书籍简述》。
...
const LEGACY_BUILTIN_DEEP_READING_PROMPT = `请忽略此前任何对话、提问、回答和中间研究结论，只根据本笔记本当前来源中的文档内容，输出一份中文《详细解读》报告。
...
await client.execute({
  ...
  args: [
    ...
    now,
    "builtin-quick-read",
    LEGACY_BUILTIN_QUICK_READ_PROMPT,
  ],
});

await client.execute({
  sql: `UPDATE summary_presets
        SET prompt = ?, description = ?, updated_at = ?
        WHERE id = ? AND prompt = ?`,
  args: [
    BUILTIN_DEEP_READING_PROMPT,
    "5000字内的结构化深度解读，覆盖论证、概念、边界与实践启发",
    now,
    "builtin-deep-reading",
    LEGACY_BUILTIN_DEEP_READING_PROMPT,
  ],
});
