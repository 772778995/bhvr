...
**在 subagent-coordination 场景下：** 当主代理使用 subagent-coordination 派遣子代理执行任务时，TDD 由执行任务的子代理遵循，不是主代理。主代理只在通过审核阶段检查子代理的 TDD 合规性（测试是否真正验证行为、是否有 TDD 痕迹）。主代理自己不写测试、不跑测试命令。
