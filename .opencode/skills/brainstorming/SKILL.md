---
name: brainstorming
description: "在创建功能、技术选型或改变行为之前使用，用于澄清真实需求、识别糟糕前提，并在需要时exploring-existing-options后形成有判断的设计方向。"
---
...
digraph brainstorming {
    ...
    "判断被改变？" [shape=diamond];
    "提出可行方法" [shape=box];
    "需要圆桌会议？" [shape=diamond];
    "并行调度三顾问" [shape=box];
    "聚合分歧，提交用户裁决" [shape=box];
    "写功能文档" [shape=box];
    "文档自审\n（内联修复）" [shape=box];
    ...
    "提出可行方法" -> "需要圆桌会议？";
    "需要圆桌会议？" -> "并行调度三顾问" [label="是（复杂任务）"];
    "需要圆桌会议？" -> "写功能文档" [label="否（极简任务）"];
    "并行调度三顾问" -> "聚合分歧，提交用户裁决";
    "聚合分歧，提交用户裁决" -> "写功能文档";
