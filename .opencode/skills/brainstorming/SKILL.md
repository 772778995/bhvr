---
name: brainstorming
description: "在创建功能、技术选型或改变行为之前使用，用于澄清真实需求、识别糟糕前提，并在需要时exploring-existing-options后形成有判断的规格方向。"
---

# 将想法转化为设计

通过自然但不迎合的协作对话，把想法压缩成可执行的规格和实现方向。

首先了解当前项目背景；如果问题涉及 build-vs-buy、技术选型或第三方方案比较，就派遣 `@explore` 子代理扫描现成方案。你不是来替用户包装偏好，你是来识别真实问题、指出糟糕前提，并把讨论压缩成明确方向。探索收敛后，直接按你的判断写出规格；除非用户明确要求停在 brainstorm/design 阶段，否则默认直接实现。当工作需要正式任务拆解时，直接在规格文档中添加实施任务部分。

<HARD-GATE>
在你完成最小必要的背景理解与按需方案探索、写出规格并通过自审之前，**不要**编写实现代码。除非用户明确要求只停在 brainstorm/design 阶段，否则规格一旦成形就直接进入实现。
</HARD-GATE>
...
- 规格不是再次征求许可的草稿，而是把当前判断固化成可执行决策。

## 反模式："这太简单了，不需要设计"

每个项目都要经过这个过程。待办事项列表、单一功能工具、配置更改——所有这些都是。"简单"项目正是未经检验的假设导致最多浪费工作的地方。规格可以很短（对于真正简单的项目几句话），但你**必须**把判断写清楚，而不是用“太简单”跳过思考。
...
6. **写功能文档** — 按功能拆分，每个功能一个目录：`docs/<类别>/<功能名>/README.md`（见下文规则）
7. **规格自审** — 快速检查占位符、矛盾、歧义、范围（见下文）
8. **按需添加实施任务** — 工作复杂时，在规格中添加实施任务部分（见下文）；简单任务直接实施
...
digraph brainstorming {
    ...
    "探索项目背景" -> "需要探索现成方案？";
    "需要探索现成方案？" -> "派遣 @explore 子代理" [label="是"];
    "需要探索现成方案？" -> "形成当前立场\n必要时纠正前提" [label="否"];
    "派遣 @explore 子代理" -> "形成当前立场\n必要时纠正前提";
    "形成当前立场\n必要时纠正前提" -> "基于立场提出关键问题";
    "基于立场提出关键问题" -> "用户回答";
    "用户回答" -> "判断被改变？";
    "判断被改变？" -> "派遣 @explore 子代理" [label="是，补充探索"];
...
**最终状态是开始实现。** 规格自审通过后，默认直接进入实现；当工作复杂时，先在功能文档中添加实施任务部分。
...
- 如果项目太大无法单个规格处理，帮助用户分解为子项目：独立的部分是什么？它们如何关联？应该按什么顺序构建？然后通过正常的设计流程进行第一个子项目的brainstorming。每个子项目有自己的规格 → 实现周期；需要任务拆解时，直接把实施任务写进对应规格。
...
2. **评估回答** — 强化、削弱还是推翻立场？
3. **决定下一步：** 立场松动 → 补充调研再提问；缺实现细节 → 继续提问；充分理解 → 直接写规格。
...
- 不要为了推进实现而强制要求先做一次 spec-only commit；功能文档可以和实现一起提交
...
> 使用 supercoder:subagent-driven-development 逐任务实施。
...
- 规格（含实施任务）本身就是实施依据
- 如果任务可以直接做，立刻开始实现
- 如果实现包含多个独立任务，使用 `supercoder:subagent-driven-development`
- 实现阶段遵循 `supercoder:test-driven-development`
- 只有当用户明确要求停在 brainstorm/design 阶段时，才在规格写完后等待
