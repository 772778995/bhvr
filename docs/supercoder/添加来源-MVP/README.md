...
> **面向智能体工作者：** 必需子技能：使用 `superpowers:subagent-driven-development` 逐任务实施此计划。步骤使用复选框（`- [ ]` / `- [x]`）跟踪状态。

**当前状态：** 未开始

**目标：** 为 `/notebook/:id` 实现“添加来源”的最小可用 MVP，覆盖 `docs/reference/notebooklm/notebook-addsource.png` 中展示的来源类型，并在每次成功添加后刷新来源列表。

**架构：** 在后端新增一组轻量的 `/api/notebooks/:id/sources/add/*` 接口，封装 `notebooklm-kit` 的来源能力，支持网站、复制文本、文件上传以及 Web/Drive 搜索发现。前端用一个轻量弹窗替换当前占位式添加动作，交互结构对齐参考图：顶部搜索区域加上文件、网站、Drive、复制文本的快捷入口。该 MVP 只聚焦可用流程和来源刷新，不做 favicon 和额外视觉打磨。
