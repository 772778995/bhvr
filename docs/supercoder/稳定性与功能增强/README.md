...
**目标：** 补全 MVP 缺失的稳定性机制，包括任务恢复、重试、日志和配额，并搭建前端 UI 骨架。

**架构：** 6 个独立任务，其中 Task 1-4 可并行，Task 5 依赖 Task 1+2 完成后做集成验证，Task 6（前端）可与任何后端任务并行。

**技术栈：** Node.js + tsx、Hono.js、Drizzle ORM + @libsql/client、`notebooklm-kit`、pino、Vue 3 + Vite + TailwindCSS
