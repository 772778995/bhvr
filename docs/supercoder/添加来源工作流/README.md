# Notebook 添加来源工作流实施计划

> **面向智能体工作者：** 必需子技能：使用 `superpowers:subagent-driven-development` 逐任务实施此计划。步骤使用复选框（`- [ ]`）语法进行跟踪。

**当前状态：** 未开始

**目标：** 在 `/notebook/:id` 中实现一个“添加来源”工作流，尽量贴近 NotebookLM 的体验，覆盖搜索网页、上传文件、添加网站、添加 Google Drive 和粘贴文本，并包含弹窗交互与来源列表刷新。

**架构：** 在后端构建一组专用的 `/api/notebooks/:id/sources/*` 来源导入接口，封装 `notebooklm-kit` 的添加与搜索 API，并返回标准化响应。前端引入一个 `AddSourceDialog` 弹窗，按来源类型提供标签页/按钮流，并在操作后触发来源刷新与处理轮询。继续保持 NotebookLM 作为来源数据真相源，不持久化本地来源内容。

**技术栈：** Vue 3 + TypeScript + TailwindCSS、Hono + TypeScript、`notebooklm-kit`（`sources.add*`、`searchWebAndWait`、`addDiscovered`、`status`）、Drizzle ORM + SQLite（仅现有扩展表）
