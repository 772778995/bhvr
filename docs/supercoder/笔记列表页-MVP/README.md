...
**目标：** 添加一个读取 NotebookLM 笔记列表的页面，在客户端展示列表，并在用户点击笔记时导航到 `/notebook/:uuid`。

**架构：** 复用服务端现有的 `listNotebooks()` NotebookLM gateway，并暴露新的 `GET /api/notebooks` 列表接口，返回标准化的笔记摘要模型。前端新增独立的笔记列表视图和路由，然后渲染可点击的笔记卡片/行，导航到已有的 notebook workbench 路由。

**技术栈：** Vue 3 + vue-router + TypeScript、Hono + TypeScript、`notebooklm-kit`、现有共享请求辅助函数
