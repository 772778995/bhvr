# NotebookLM Single-Source Research Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 `notebook/:id` 工作台从 stub 数据重构为 NotebookLM 直连模式，并实现服务端驱动的自动研究、SSE 实时观测、以及只保存最终正式报告的扩展层。

**Architecture:** Notebook 原生数据全部改为通过后端 gateway 直连 NotebookLM；自动研究由服务端 orchestrator 在内存 runtime registry 中推进，并通过 SSE 向前端暴露步骤级状态；本地数据库缩减为仅保存当前正式研究报告，不再保存 Notebook 原生问答副本。

**Tech Stack:** Vue 3 + vue-router + TypeScript + Vite, Hono + TypeScript, notebooklm-kit, Drizzle ORM + SQLite, Server-Sent Events
