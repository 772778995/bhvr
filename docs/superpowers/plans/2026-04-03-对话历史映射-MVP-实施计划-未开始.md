# Notebook 工作台聊天无持久化实施计划

> **面向智能体工作者：** 必需子技能：使用 `superpowers:subagent-driven-development` 逐任务实施此计划。步骤使用复选框（`- [ ]`）语法进行跟踪。

**当前状态：** 未开始

**目标：** 让 `/notebook/:id` 能从工作台向 NotebookLM 发送聊天消息，但不增加任何本地持久化层。

**架构：** 聊天状态只保留在浏览器当前页面会话中。服务端新增一个轻量的发送接口，把提示词、启用的来源 ID 和可选的内存续上下文转发给 NotebookLM。我们明确不在 SQLite 中存储对话或消息记录，也不尝试在刷新后重建历史。

**技术栈：** Vue 3 + TypeScript、Hono、`notebooklm-kit` `generation.chat`、`tsx` Node test runner
