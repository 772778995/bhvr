# Notebook Sources Sidebar + Source Toggle Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Upgrade the left sources sidebar to match NotebookLM behavior by showing source icons, opening web sources by click, and supporting per-source enabled/disabled toggles that affect research calls.

**Architecture:** Keep NotebookLM as source-of-truth for source records, and add a tiny extension layer in our DB for per-notebook source enablement state (`enabled` default true). Backend merges NotebookLM sources with local toggle state in read APIs and exposes a toggle endpoint. Frontend renders icon + click behavior + toggle control in `SourcesPanel`, and refreshes sources after toggle changes.

**Tech Stack:** Vue 3 + TypeScript + TailwindCSS, Hono + TypeScript, Drizzle ORM + SQLite, notebooklm-kit
